...
def test_monitor_admin_create_pipeline_calls_repository(mock_run_repo, mock_step_repo):
    ...
    response = client.post(
        ...
        content=(
            "name=production_log&table=PRODUCTION_LOG"
            "&source_where_template=LOG_DATE+%3E%3D+:data_interval_start+AND+LOG_DATE+%3C+:data_interval_end"
...
def test_monitor_admin_create_pipeline_accepts_source_bind_params(
    mock_run_repo, mock_step_repo
):
    settings = _make_settings()
    admin_repo = MagicMock()
    app = create_app(
    ...
    response = client.post(
        ...
        content=(
            "name=production_log&table=PRODUCTION_LOG"
            "&source_where_template=TRAN_YEAR+%3D+:year+AND+TRAN_MONTH+%3D+:month"
            "&source_bind_params=%7B%22year%22%3A%22%7B%7B+data_interval_start.year+%7D%7D%22%2C%22month%22%3A%22%7B%7B+data_interval_start.month+%7D%7D%22%7D"
            "&strategy=full&schedule=0+2+%2A+%2A+%2A"
        ),
        headers={"content-type": "application/x-www-form-urlencoded"},
        follow_redirects=False,
    )
    ...
    admin_repo.create_pipeline.assert_called_once()
    created = admin_repo.create_pipeline.call_args.args[0]
    assert created.source_where_template == "TRAN_YEAR = :year AND TRAN_MONTH = :month"
    assert created.source_bind_params == '{"month":"{{ data_interval_start.month }}","year":"{{ data_interval_start.year }}"}'


def test_monitor_admin_create_pipeline_rejects_bind_params_without_template(
    mock_run_repo, mock_step_repo
):
    settings = _make_settings()
    admin_repo = MagicMock()
    ...
    response = client.post(
        ...
        content=(
            "name=production_log&table=PRODUCTION_LOG"
            "&source_bind_params=%7B%22year%22%3A2026%7D"
            "&strategy=full&schedule=0+2+%2A+%2A+%2A"
        ),
        headers={"content-type": "application/x-www-form-urlencoded"},
        follow_redirects=False,
    ...
    admin_repo.create_pipeline.assert_not_called()
...
def test_admin_pipeline_rest_create_rejects_bind_params_without_template(
    tmp_path, mock_run_repo, mock_step_repo
):
    settings = _make_settings()
    ...
    assert "source_bind_params" in response.json()["detail"]
