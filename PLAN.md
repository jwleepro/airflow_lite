...
- `T-026` 5단계 결과 평가: 운영 액션 버튼 체계가 Home/DAGs/DAG Details/Dag Run 흐름에 맞게 정착됐는지 검토한다.

### 다음

- `T-026` step 1~5 구현 결과를 묶어 잔여 Airflow 차이와 후속 범위를 재정리한다.
...
1. `T-026` step 1~5 결과를 종합 평가하고 남은 Airflow 차이를 재정리한다.
2. `T-026` 지원하지 않는 Airflow 액션(`pause`, `clear`, `mark success`)을 별도 시스템 기준으로 도입할지 결정한다.
3. M4 마일스톤 후속 범위 검토 및 M5 정의.
...
Airflow 3 스타일 UI shell과 운영 액션 버튼 체계 구현을 묶어 평가하고, 이후 남은 기능 차이와 M5 범위를 재정의한다.
