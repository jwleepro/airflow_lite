...
def get_browse_backfills_page(request: Request, language: str = Depends(get_language)):
    ...
    return HTMLResponse(
        render_browse_backfills_page(
            ...
            selected_dag=params.get("dag_id", ""),
        )
    )


@router.get(MONITOR_BROWSE_JOBS_PATH, response_class=HTMLResponse)
def get_browse_jobs_page(request: Request, language: str = Depends(get_language)):
    params = request.query_params
    return HTMLResponse(
        render_browse_jobs_page(
...
def get_browse_audit_logs_page(request: Request, language: str = Depends(get_language)):
    params = request.query_params
    return HTMLResponse(
        render_browse_audit_logs_page(
            ...
            language=language,
            search_query=params.get("q", ""),
            selected_action=params.get("action", ""),
...
def get_browse_task_instances_page(request: Request, language: str = Depends(get_language)):
    params = request.query_params
    return HTMLResponse(
        render_browse_task_instances_page(
...
def get_browse_dag_runs_page(request: Request, language: str = Depends(get_language)):
    params = request.query_params
    return HTMLResponse(
        render_browse_dag_runs_page(
